...
class AuthController extends Controller {
    ...
    public function signup(Request $request){
        ...
        if($validators->fails()){
            ...
            $user->save();
            auth()->login($user);
            return redirect()->intended(route('dashboard.agentscolarite','en'))->with('message','Registration was successfull !');            
        }
    }
    ...
    /*public function authenticate(Request $request){
        ...
        if($validators->fails()){
            ...
            if(Auth::attempt($credentials)){
                $user = Auth::user();
                if ($user->role == 1) {
                    return redirect()->intended(route('dashboard.etudiant'))->with('message', 'Welcome back!');
                } elseif ($user->role == 2) {
                    return redirect()->intended(route('dashboard.agentscolarite'))->with('message', 'Welcome back!');
                } elseif ($user->role == 3) {
                    return redirect()->intended(route('dashboard.agentpedagogique'))->with('message', 'Welcome back!');
                }
            } else {
                return redirect()->route('login')->with('message', 'Connexion echoué, Veuillez verifier le mail ou le mot de passe');
            }
        }
    }*/
